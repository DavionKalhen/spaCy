...
def test_issue5082():
    ...
    nlp = English()
    vocab = nlp.vocab
    array1 = nlcpy.asarray([0.1, 0.5, 0.8], dtype=nlcpy.float32)
    array2 = nlcpy.asarray([-0.2, -0.6, -0.9], dtype=nlcpy.float32)
    array3 = nlcpy.asarray([0.3, -0.1, 0.7], dtype=nlcpy.float32)
    array4 = nlcpy.asarray([0.5, 0, 0.3], dtype=nlcpy.float32)
    array34 = nlcpy.asarray([0.4, -0.05, 0.5], dtype=nlcpy.float32)
    ...
    ops = get_current_ops()
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_1[0]), array1)
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_1[1]), array2)
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_1[2]), array3)
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_1[3]), array4)
    nlp.add_pipe("merge_entities")
    parsed_vectors_2 = [t.vector for t in nlp(text)]
    assert len(parsed_vectors_2) == 3
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_2[0]), array1)
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_2[1]), array2)
    nlcpy.testing.assert_array_equal(ops.to_nlcpy(parsed_vectors_2[2]), array34)
